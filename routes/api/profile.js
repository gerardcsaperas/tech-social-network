...
// @access  Private
router.post(
    '/', [auth, [body('status', 'Status is required').notEmpty(), body('skills', 'Skills is required').notEmpty()]],
    async(req, res) => {
...
router.put(
    '/experience', [
        auth, [
            ...
            body('location', 'title is required').notEmpty(),
            body('from', 'Fromt date is required').notEmpty()
        ]
    ],
    async(req, res) => {
        const errors = validationResult(req);
...
router.put(
    ...
    async(req, res) => {
        ...
        const newStudy = { degree, school, fieldOfStudy, from, to, current, description };

        try {
            const profile = await Profile.findOne({ user: req.user.id });

            profile.education.unshift(newStudy);
...
// @access  Private
router.delete('/education/:study_id', auth, async(req, res) => {
    try {
        const profile = await Profile.findOne({ user: req.user.id });

        // Get remove index
        const removeIndex = profile.education.map((item) => item.id).indexOf(req.params.study_id);
